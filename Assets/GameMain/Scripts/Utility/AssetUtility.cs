...
namespace GFLearning
{
    public static class AssetUtility
    {
        ...
        public static string GetFontAsset(string assetName)
        {
            ...
            string ttfPath = Utility.Text.Format("Assets/GameMain/Fonts/{0}.ttf", assetName);
            string otfPath = Utility.Text.Format("Assets/GameMain/Fonts/{0}.otf", assetName);

            if (System.IO.File.Exists(ttfPath))
            {
                return ttfPath;
            }
            else if (System.IO.File.Exists(otfPath))
            {
                return otfPath;
            }
            else
            {
                Log.Error("No Font Assets Exists");
